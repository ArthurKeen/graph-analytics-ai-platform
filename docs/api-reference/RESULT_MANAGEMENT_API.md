...
**Module:** `graph_analytics_ai.results`
...
**Example:**
```python
from graph_analytics_ai import get_db_connection, results
...
**Module:** `graph_analytics_ai.queries`
...
- `filter2` (Optional[str]): Optional AQL filter for collection2 (e.g., `"w.component_id == 'persons/xxx'"`)
...
**Example:**
```python
from graph_analytics_ai import get_db_connection, queries
...
- `vertex_collection` (str): Vertex collection name if including details. Default: `'persons'`
...
for result in results:
    print(f"{result.get('FullName', 'Unknown')}: {result['pagerank_influence']}")
```
...
- `vertex_collection` (str): Vertex collection name (e.g., `'persons'`). Default: `'persons'`
- `result_filter` (Optional[str]): Optional AQL filter for results (e.g., `"r.pagerank_influence >= 0.000002"`)
- `fields` (Optional[List[str]]): Optional list of vertex fields to include. Defaults to `['NameEx.GdValue.FullName', 'PrimarySpecialties', 'Emails']`
...
for result in results:
    print(f"{result.get('FullName')}: {result['result_data']['pagerank_influence']}")
```
...
**Module:** `graph_analytics_ai.export`
...
- `vertex_collection` (str): Vertex collection name if joining. Default: `'persons'`
- `vertex_fields` (Optional[List[str]]): Optional list of vertex fields to include
...
**Example:**
```python
from graph_analytics_ai import get_db_connection, export
